...
Item {
    ...
    Component.onCompleted: {
        // Atomik OS: controlla LOGO prima della lista NerdFont
        Proc.runCommand(null, ["sh", "-c", ". /etc/os-release && echo $LOGO"], (logoOutput, logoExitCode) => {
            if (!root || logoExitCode !== 0) return
            const logo = logoOutput ? logoOutput.trim() : ""
            if (logo) {
                if (logo === "cachyos") {
            ...
            // Fallback: NerdFont per distro note
